...
#    let this script apply it. versionCode is NOT touched here — bump it in the
#    recipe above the highest already uploaded (Play Console -> App bundle explorer).
...
if [ -n "${VN:-}" ] && [ -f "$APP_GRADLE" ]; then
  ...
  echo "  versionName    -> ${VN}  (from package.json)"
