...
import { Home, Shield, Users, CalendarDays, ChevronLeft, ChevronRight, Search, BookOpen, ExternalLink, ArrowLeftRight } from "lucide-react";
// Bundled, not hot-linked — the app has no INTERNET permission (see CLAUDE.md),
// so a live @import from Google Fonts silently fails on device and every
// Oswald-styled figure falls back to a generic sans-serif with tight/negative
// kerning, which is what was making digits collide with "%" signs and
// adjacent labels crowd each other on the shot fingerprint. One variable-weight
// woff2 (400-700, ~21KB) covers every weight this file uses. License:
// src/assets/fonts/OSWALD-OFL.txt (SIL Open Font License 1.1).
import oswaldWoff2 from "./assets/fonts/oswald-var.woff2";
...
export default function CourtsideApp() {
  ...
  return (
    <div style={{ background: C.page, color: C.text, height: "100%", display: "flex", flexDirection: "column", position: "relative", overflow: "hidden", fontFamily: BODY }}>
      <style>{`
        /* Bundled locally — see the oswaldWoff2 import above. Roboto isn't
           bundled: BODY's system-ui fallback already resolves to Android's
           native Roboto, so there's nothing a network fetch would add. */
        @font-face {
          font-family: 'Oswald';
          font-style: normal;
          font-weight: 400 700;
          font-display: swap;
          src: url('${oswaldWoff2}') format('woff2');
        }
      ...
      {stack ? (
        ...
        <div key={tab} className="cs-fade" style={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
          {tab === "home" && <HomeScreen open={open} onSearch={() => setSearch(true)} theme={theme} setTheme={chooseTheme} goLearn={() => { setCompareEdit(null); setStack(null); setTab("learn"); }} />}
...
// ============================================================================
function HomeScreen({ open, onSearch, theme, setTheme, goLearn }) {
  ...
  return (
    <>
      <Masthead title="Courtside" sub="2026 Season" onSearch={onSearch} right={
        <button onClick={() => setAboutOpen(o => !o)} aria-label="About" style={{
          width: 27, height: 27, borderRadius: 99, border: `1px solid ${C.border}`, background: C.card, color: C.sec,
          display: "flex", alignItems: "center", justifyContent: "center", cursor: "pointer",
          fontFamily: DISPLAY, fontWeight: 700, fontSize: 13,
        }}>i</button>
      } />
      <div style={{ maxHeight: aboutOpen ? 600 : 0, overflow: "hidden", transition: "max-height .28s ease", background: C.card, borderBottom: aboutOpen ? `1px solid ${C.border}` : "none", flexShrink: 0 }}>
        <div style={{ padding: "12px 14px 16px" }}>
          <AboutContent theme={theme} setTheme={setTheme} />
        </div>
      </div>
      <ScoreRail open={open} />
      <div className="noscroll" style={{ flex: 1, overflowY: "auto", padding: "0 14px 12px" }}>
...
// Was a standalone "About" tab; now the content behind Today's (i) button —
// About was rarely tapped from anywhere else, so it no longer needs a whole
// tab of its own. Same content, no Masthead/scroll wrapper of its own since
// it now lives inside HomeScreen's collapsible panel.
function AboutContent({ theme, setTheme }) {
  return (
    <>
...
// ============================================================================
// LEARN — "Intro to the WNBA". Built from an interactive mockup reviewed and
// refined over several rounds before any of this was written (nav swap, the
// four sub-tabs, the horizontal-scroll fix on the facts strip, the one-time
// banner). Replaces the old standalone About tab; About itself moved to the
// (i) button on Today (see HomeScreen/AboutContent).
const LEARN_BANNER_KEY = "courtside:seenLearnPrompt";
function learnBannerSeen() { try { return localStorage.getItem(LEARN_BANNER_KEY) === "1"; } catch { return false; } }
function setLearnBannerSeen(v) { try { localStorage.setItem(LEARN_BANNER_KEY, v ? "1" : "0"); } catch {} }

function LearnBanner({ goLearn }) {
  const [seen, setSeen] = useState(learnBannerSeen());
  if (seen) return null;
  const dismiss = () => { setLearnBannerSeen(true); setSeen(true); };
  return (
    <div style={{ position: "relative", margin: "10px 0 2px", padding: "12px 30px 12px 13px", background: C.card, border: `1px solid ${C.border}`, borderLeft: `3px solid ${C.accent}`, borderRadius: 2 }}>
      <button onClick={dismiss} aria-label="Dismiss" style={{ position: "absolute", top: 8, right: 8, width: 20, height: 20, border: "none", background: "none", color: C.mute, cursor: "pointer", fontSize: 15, lineHeight: 1, display: "flex", alignItems: "center", justifyContent: "center" }}>&times;</button>
      <div style={{ fontFamily: DISPLAY, fontSize: 13, fontWeight: 600, color: C.text, marginBottom: 3 }}>New to the WNBA?</div>
      <p style={{ fontSize: 10.5, lineHeight: 1.5, color: C.sec, margin: "0 0 8px" }}>Rules, positions, and how to read the stats on this page — about six minutes.</p>
      <button onClick={() => { dismiss(); goLearn(); }} style={{ display: "inline-flex", alignItems: "center", gap: 4, fontFamily: DISPLAY, fontSize: 10.5, fontWeight: 700, letterSpacing: .4, textTransform: "uppercase", color: C.accent, background: "none", border: "none", padding: 0, cursor: "pointer" }}>
        Learn the game →
      </button>
    </div>
  );
...
function LearnFacts({ items }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 0, borderTop: `1px solid ${C.border}`, margin: "8px 0 4px" }}>
      {items.map(([k, v], i) => (
        <div key={k} style={{ padding: "9px 8px", borderBottom: `1px solid ${C.border}`, borderRight: (i + 1) % 3 ? `1px solid ${C.border}` : "none" }}>
          <div style={{ fontSize: 7.5, letterSpacing: .8, textTransform: "uppercase", color: C.sec, fontWeight: 700 }}>{k}</div>
          <div style={{ fontFamily: DISPLAY, fontSize: 14, fontWeight: 600, color: C.text, marginTop: 1, ...agate }}>{v}</div>
        </div>
      ))}
    </div>
  );
}
function LearnP({ children }) { return <p style={{ fontSize: 11.5, lineHeight: 1.65, color: C.sec, padding: "8px 0 2px", margin: 0 }}>{children}</p>; }
function LearnLead({ children }) { return <p style={{ fontSize: 12, lineHeight: 1.6, color: C.sec, padding: "10px 0", margin: 0, borderBottom: `1px solid ${C.border}` }}>{children}</p>; }
function LearnB({ children }) { return <b style={{ color: C.text, fontWeight: 700 }}>{children}</b>; }

function LearnRoleGrid({ roles }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 0, margin: "8px 0 4px", borderTop: `1px solid ${C.border}` }}>
      {roles.map(([tag, name, desc], i) => (
        <div key={tag} style={{ padding: "9px 8px 9px 0", borderRight: i < roles.length - 1 ? `1px solid ${C.border}` : "none" }}>
          <span style={{ display: "inline-block", fontFamily: DISPLAY, fontSize: 10, fontWeight: 700, letterSpacing: .5, color: C.onRule, background: C.rule, padding: "1px 6px", borderRadius: 2, marginBottom: 5 }}>{tag}</span>
          <p style={{ fontSize: 10, lineHeight: 1.5, color: C.sec, margin: 0 }}><LearnB>{name}</LearnB><br />{desc}</p>
        </div>
      ))}
    </div>
  );
}
function LearnTermRow({ term, def }) {
  return (
    <div style={{ padding: "8px 0", borderBottom: `1px solid ${C.border}` }}>
      <div style={{ fontFamily: DISPLAY, fontSize: 11.5, fontWeight: 600, color: C.text }}>{term}</div>
      <div style={{ fontSize: 10.5, lineHeight: 1.5, color: C.sec, marginTop: 2 }}>{def}</div>
    </div>
  );
}
function LearnGlossGrid({ items }) {
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0, margin: "8px 0 4px", borderTop: `1px solid ${C.border}` }}>
      {items.map(([term, def], i) => {
        const odd = i % 2 === 0;
        const stripe = i % 4 < 2;
        return (
          <div key={term} style={{
            padding: odd ? "8px 9px 8px 0" : "8px 0 8px 9px",
            borderBottom: `1px solid ${C.border}`, borderRight: odd ? `1px solid ${C.border}` : "none",
            background: stripe ? C.stripe : "transparent",
          }}>
            <div style={{ fontFamily: DISPLAY, fontSize: 10.5, fontWeight: 700, letterSpacing: .3, color: C.text }}>{term}</div>
            <div style={{ fontSize: 9.5, lineHeight: 1.4, color: C.sec, marginTop: 2 }}>{def}</div>
          </div>
        );
      })}
    </div>
...
function LearnStatLine({ term, def }) {
  return (
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "baseline", padding: "9px 0", borderBottom: `1px solid ${C.border}`, gap: 10 }}>
      <div style={{ fontFamily: DISPLAY, fontSize: 12.5, fontWeight: 600, color: C.accent, flexShrink: 0, width: 62, ...agate }}>{term}</div>
      <div style={{ fontSize: 10.8, lineHeight: 1.5, color: C.sec, textAlign: "left" }}>{def}</div>
    </div>
  );
}
...
function LearnBasics() {
  return (
    <>
      <LearnFacts items={[["Teams", "15"], ["Quarters", "4×10 min"], ["Shot clock", "24s"], ["On court", "5v5"], ["Season", "May–Sep"]]} />
      <LearnLead><LearnB>New to women's basketball?</LearnB> Same court, same hoop height, same core rules as the game you already half-know — here's the rest, in six minutes.</LearnLead>

      <SectionHead action="01 / 03">Scoring</SectionHead>
      <LearnP>A basket inside the arc is worth <LearnB>2 points</LearnB>, beyond it <LearnB>3</LearnB>, and a free throw — awarded after a foul — is worth <LearnB>1</LearnB>.</LearnP>
      <LearnP>Most points after four quarters wins. A tie forces a <LearnB>5-minute overtime</LearnB> — as many as it takes. Every extra period is a full 5 minutes, no matter the score; there's no golden point.</LearnP>

      <SectionHead action="02 / 03">Clock &amp; possession</SectionHead>
      <LearnP>Four <LearnB>10-minute quarters</LearnB>, 40 minutes of play before overtime.</LearnP>
      <LearnP>A <LearnB>24-second shot clock</LearnB>: get a shot up in time or the ball turns over. It resets to 14 seconds after an offensive rebound.</LearnP>
      <LearnP><LearnB>8 seconds</LearnB> to move the ball past halfcourt, or it's a turnover.</LearnP>
      <LearnP>Each team gets a handful of timeouts to stop the clock — coaches lean on them late, which is why close finishes can take a while to actually finish.</LearnP>

      <SectionHead action="03 / 03">Fouls</SectionHead>
      <LearnP><LearnB>6 personal fouls</LearnB> disqualifies a player for the rest of the game.</LearnP>
      <LearnP>Team fouls reset each quarter. From the <LearnB>5th team foul</LearnB> on, every foul sends the other team to the free-throw line — no more "1-and-1."</LearnP>
      <LearnP>A flagrant or technical foul carries free throws <i>and</i> lets the fouled team keep the ball. An offensive foul — a charge, for instance — is the one that sends nobody to the line, just a turnover.</LearnP>
    </>
  );
}
function LearnPositions() {
  return (
    <>
      <SectionHead>The three roles</SectionHead>
      <LearnP>Modern rosters are more fluid than the old 1-through-5, so this app — like most broadcasts — groups players into three roles:</LearnP>
      <LearnRoleGrid roles={[
        ["G", "Guard", "Ball-handling, perimeter shooting, setting up the offense."],
        ["F", "Forward", "The versatile middle — scores inside and out, rebounds."],
        ["C", "Center", "Anchors the paint on both ends."],
      ]} />
      <LearnP>That's exactly the tag next to a player's name everywhere in this app.</LearnP>

      <SectionHead>On the broadcast, you'll also hear</SectionHead>
      <LearnTermRow term="Point guard" def="The primary ball-handler — usually leads the team in assists." />
      <LearnTermRow term="Wing" def="A perimeter scorer and on-ball defender — the modern in-between of guard and forward." />
      <LearnTermRow term="Post (or “the 5”)" def="Plays with their back to the basket — usually leads in rebounds and blocks." />
      <LearnTermRow term="Stretch four" def="A forward who shoots 3s instead of staying inside — increasingly common." />
      <LearnP>None of this is official. Broadcasts and this app both simplify to <LearnB>G / F / C</LearnB> — that's genuinely all you need to follow along.</LearnP>
    </>
  );
}
function LearnSeason() {
  return (
    <>
      <SectionHead>The season</SectionHead>
      <LearnP>Regular season runs <LearnB>mid-May to September</LearnB>, roughly 40 games a team.</LearnP>
      <LearnP><LearnB>All-Star break</LearnB>: one weekend in July, the league's best in one exhibition game.</LearnP>
      <LearnP>Playoffs seed the whole league on <LearnB>one ladder</LearnB> — no conference split. Win your round to advance; each round is a longer series than the last.</LearnP>

      <SectionHead>Awards</SectionHead>
      <LearnP>Handed out at season's end: MVP, Rookie of the Year, Defensive Player of the Year, Sixth Player of the Year, Most Improved Player, plus All-WNBA and All-Defensive teams.</LearnP>
      <LearnP>Win one, and it shows up on that player's page under <LearnB>Background</LearnB>.</LearnP>

      <SectionHead>Standings</SectionHead>
      <LearnP>Teams are ranked by <LearnB>winning percentage</LearnB>, not raw win count — schedules can be a game or two off between teams, so percentage is the fair comparison.</LearnP>
    </>
  );
}
function LearnStats() {
  return (
    <>
      <SectionHead action="01 / 03">Box score</SectionHead>
      <LearnP>The letters under every player and team on this app, decoded:</LearnP>
      <LearnGlossGrid items={[
        ["GP", "Games played this season."], ["MPG", "Minutes per game — a signal of trust and role."],
        ["PTS", "Points scored."], ["REB", "Rebounds — offense and defense combined."],
        ["AST", "Assists — passes that directly lead to a made basket."], ["STL", "Steals — live-ball takeaways."],
        ["BLK", "Blocked shots."], ["TOV", "Turnovers — giveaways."],
      ]} />

      <SectionHead action="02 / 03">Shooting</SectionHead>
      <LearnP>FG% / 3P% / FT% are simply <LearnB>makes ÷ attempts</LearnB> — the most literal "how well are they shooting" number there is.</LearnP>
      <LearnStatLine term="TS%" def="True shooting — one number for scoring efficiency, weighing a 3 fairly and a free throw as the cheap point it is." />

      <SectionHead action="03 / 03">Advanced</SectionHead>
      <LearnStatLine term="USG%" def="Usage — the share of their team's plays a player finishes on the floor. High usage means the offense runs through them." />
      <LearnStatLine term="Percentile" def="Ranks a player against others at the same position. 75th in rebounding = out-rebounds 3 of every 4 peers." />

      <div style={{ margin: "10px 0 4px", padding: "12px 12px 13px", background: C.stripe, border: `1px solid ${C.border}`, borderRadius: 3 }}>
        <div style={{ fontSize: 8, letterSpacing: 1, textTransform: "uppercase", color: C.sec, fontWeight: 700, marginBottom: 6 }}>Reading a line</div>
        <div style={{ fontFamily: DISPLAY, fontSize: 15, fontWeight: 600, color: C.text, marginBottom: 6, ...agate }}>26 PTS · 9 REB · 3 AST on 53% shooting</div>
        <div style={{ fontSize: 11, lineHeight: 1.55, color: C.sec }}>A big, <b style={{ color: C.good, fontWeight: 700 }}>efficient</b> night — high volume <i>and</i> high efficiency. A 26-point game on 30 shots reads very differently to one on 15.</div>
      </div>
    </>
  );
}
...
function LearnScreen() {
  ...
  const tabs = ["Basics", "Positions", "Season", "Stats"];
  const swipe = useSwipeTabs(tabs, sub, setSub);
  return (
    <>
      <Masthead title="Learn" sub="intro to the WNBA" />
      <SubTabs tabs={tabs} active={sub} set={setSub} />
      <div key={sub} className={"noscroll " + swipe.dir} {...swipe.handlers} style={{ flex: 1, overflowY: "auto", overflowX: "hidden", padding: "0 14px 14px" }}>
        {sub === "Positions" ? <LearnPositions /> : sub === "Season" ? <LearnSeason /> : sub === "Stats" ? <LearnStats /> : <LearnBasics />}
      </div>
    </>
  );
}
    ...
    <g transform={`translate(${x} ${y})`}>
      ...
      {mode === "eff" && delta != null && (
        <text textAnchor="middle" y={4.5} {...HALO}
          style={{ fontSize: 1.3, fontWeight: 700, letterSpacing: 0.06, fill: dc }}>
...
function TabBar({ tab, setTab }) {
  const items = [["home", Home, "Today"], ["teams", Shield, "Teams"], ["players", Users, "Players"], ["schedule", CalendarDays, "Schedule"], ["learn", BookOpen, "Learn"]];
  return (
